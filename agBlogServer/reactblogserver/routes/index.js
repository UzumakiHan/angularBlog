...
//查询数据
router.get(BASE_URL+"/findid", (req, res) => {
  let id = req.query.id;
  const array = [];
  ...
  connection.query(sqlStr, (error, results, fields) => {
    if (error) {
      ...
      // }
     // console.log(results)
      const htmlstr = new Buffer(results[0].content.data).toString()
      //console.log(htmlstr)
      // console.log(new Buffer(results[0].content.data).toString())
      //  for(var i = 0;i < results.length;i++){
      //    console.log(results[i])
      //  }
      res.json({ success_code: 200, data: results,htmlstr:htmlstr});
...
//注册
router.post(BASE_URL+'/reqregister', (req, res) => {
  ...
  connection.query(sqlStr, (error, results, fields) => {
    if (results[0]) {//用户已经存在
      ...
      connection.query(addSql, addSqlParams, (error, results, fields) => {
        if (error) {
          res.json({ success_code: 0, message: "注册失败" })
        } else {
...
//登录
router.get(BASE_URL+'/reqlogin', (req, res) => {
...
//文章发布
router.post(BASE_URL+'/release', (req, res) => {
  ...
  connection.query(sqlStr, addSqlParams, (error, results, fields) => {
    if (error) {
      res.json({ success_code: 0, message: "发布文章失败" });
    } else {
      console.log(results);
...
router.get(BASE_URL+'/getallcontent', (req, res) => {
  ...
  connection.query(sqlStr, (error, results, fields) => {
    if (error) {
      ...
      // console.log(array);
      res.json({ success_code: 200, data: pageFilter(results, pageNum, pageSize),message:pageFilter(array, pageNum, pageSize)});

    }
  })
})
//获取热门文章
router.get(BASE_URL+'/getHotArticle', (req, res) => {
...
//获取个人文章信息
router.get(BASE_URL+'/getowncontent', (req, res) => {
...
//修改个人资料
router.post(BASE_URL+'/modifyingdata', (req, res) => {
    ...
    if (error) {
      ...
      connection.query(otherSqlStr, (err, result, field) => {
        if (err) {
          ...
          console.log(result);
          res.json({ success_code: 200, message: "修改成功", data: result });
        }
      })
...
//获取个人信息；
router.get(BASE_URL+'/getinfo', (req, res) => {
...
router.post(BASE_URL+'/submitImage', (req, res) => {
...
router.post(BASE_URL+'/blogUserImage', (req, res) => {
  const { image, username } = req.body;
  console.log(image, username);
...
//获取图片
router.post(BASE_URL+'/getimage', (req, res) => {
...
//删除文章
router.post(BASE_URL+'/delarticle', (req, res) => {
  const id = req.body.id;
  console.log(id);
  const sqlStr = "DELETE FROM blog WHERE id = '" + id + "'";
  connection.query(sqlStr, (error, results, fields) => {
    if (error) {
      res.json({ success_code: 0, message: "删除失败" });
    } else {
...
//编辑文章
router.post(BASE_URL+'/editarticle', (req, res) => {
  ...
  connection.query(sqlStr, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ success_code: 0, message: "修改失败" });
    } else {
      console.log(results)
...
//修改密码接口
router.post(BASE_URL+'/changepassword', (req, res) => {
  const id = req.body.id;
  ...
  connection.query(sqlStr, (error, results, fields) => {
    if (error) {
      ...
      console.log(results);
      res.json({ success_code: 200, message: "修改密码成功，请重新登录" });
    }
  })
})

//阅读数
router.get(BASE_URL+'/readconut', (req, res) => {
...
//评论
router.post(BASE_URL+'/commit', (req, res) => {
...
//获取相对于的评论
router.get(BASE_URL+'/getcommit', (req, res) => {
...
router.post(BASE_URL+'/getmycomment', (req, res) => {
...
//修改评论状态
router.post(BASE_URL+'/modifyStatus', (req, res) => {
  let id = req.body.id;
  let status = req.body.status;
...
//获取用户卡片
router.post(BASE_URL+'/getUserCard', (req, res) => {
...
//用户关注
router.post(BASE_URL+'/careUser', (req, res) => {
  let bloguser = req.body.bloguser;
...
//获取用户关注列表
router.post(BASE_URL+'/getUserList', (req, res) => {
  let bloguser = req.body.bloguser;
  console.log(bloguser);
...
//取消关注的用户
router.post(BASE_URL+'/cancleCare', (req, res) => {
  let carebloguser = req.body.carebloguser;
...
//更新状态
router.post(BASE_URL+'/modifyuserstatus', (req, res) => {
  let id = req.body.id;
  let status = req.body.status;
  console.log(id, status);
